    ...
    def create(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            if not data.get("configuration"):
                # Nombre visible de la configuración: el SKU lo genera el trigger en BD;
                # usamos el nombre del ítem o un identificador estable.
                cfg_label = (data.get("name") or "").strip()
                if not cfg_label:
                    cfg_label = f"AUTO-{uuid.uuid4().hex[:12]}"
