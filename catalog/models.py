...
class Service(models.Model):
    ...
    min_package_purchase = models.IntegerField()
    price = models.ForeignKey(
        "price.Price", db_column="price", to_field="code", on_delete=models.PROTECT
    )
    provider = models.ForeignKey(
        Provider, db_column="provider", on_delete=models.PROTECT
    )
    type = models.ForeignKey("ItemType", db_column="type", on_delete=models.PROTECT)
    item_group = models.ForeignKey(
        "ItemGroup", db_column="item_group", on_delete=models.PROTECT
    )
    category = models.ForeignKey(
        "ItemCategory", db_column="category", on_delete=models.PROTECT
    )
    url = models.CharField(max_length=255, null=True, blank=True)
    is_active = models.BooleanField(default=True)
